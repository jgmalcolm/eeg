...
for loc = 1:nlocs
  ...
  for freq = freqs
    for amp = amps
      for dur = durs
        d = d_stim(d_stim.Frequency==freq & d_stim.Amplitude==amp & d_stim.Duration==dur, :);
  ...
  pp_pos(cellfun(@(pval) isempty(pval), pp_pos(:,end)),:) = [];
  pp_neg(cellfun(@(pval) isempty(pval), pp_neg(:,end)),:) = [];
